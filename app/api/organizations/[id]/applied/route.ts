...
import { GetOrganizationAppliedCompaniesUseCase } from '../../../../../src/application/use-cases/GetOrganizationAppliedCompaniesUseCase';
...
const getOrganizationAppliedCompaniesUseCase = new GetOrganizationAppliedCompaniesUseCase(
  proposalRepository,
  companyRepository
);
...
export async function GET(
  ...
  try {
    ...
    const appliedCompanies = await getOrganizationAppliedCompaniesUseCase.execute(id);

    return NextResponse.json(appliedCompanies, { status: 200 });
  } catch (error) {
    ...
    logError(error instanceof Error ? error : new Error('Unknown error'), {
      endpoint: '/api/organizations/[id]/applied',
      method: 'GET',
