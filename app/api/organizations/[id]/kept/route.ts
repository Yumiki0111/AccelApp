...
import { GetOrganizationKeptCompaniesUseCase } from '../../../../../src/application/use-cases/GetOrganizationKeptCompaniesUseCase';
...
const getOrganizationKeptCompaniesUseCase = new GetOrganizationKeptCompaniesUseCase(companyRepository);
...
export async function GET(
  ...
  try {
    ...
    const keptCompanies = await getOrganizationKeptCompaniesUseCase.execute(id);

    return NextResponse.json(keptCompanies, { status: 200 });
  } catch (error) {
    ...
    logError(error instanceof Error ? error : new Error('Unknown error'), {
      endpoint: '/api/organizations/[id]/kept',
      method: 'GET',
